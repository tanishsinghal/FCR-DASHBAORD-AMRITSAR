...
crit_df = filtered_df.groupby("Tehsil").sum(numeric_only=True).reset_index()

# Define logic
crit_df["Pending"] = (
    crit_df["Total No. of Villages Received by Dist. from SoI"] -
    crit_df["Villages where ground truthing completed & sent back to SoI"]
)
...
# Flag critical (you can tune this)
critical = crit_df[
    (crit_df["Completion %"] < 50) & (crit_df["Pending"] > 20)
]

if critical.empty:
    st.success("✅ No critical tehsils")
else:
    st.error("⚠️ Critical Tehsils Detected")
    st.dataframe(critical.sort_values("Pending", ascending=False), use_container_width=True)


# ==============================
# 🔁 DAY-TO-DAY DELTA
# ==============================
st.subheader("🔁 Daily Growth (Delta)")
...
fig = px.bar(
    ...
    x="Date",
    y="Delta Completed",
    title="Daily Growth in Completed Villages"
)
...
# 🎯 PERFORMANCE RANKING
# ==============================
st.subheader("🎯 Tehsil Performance Ranking")

rank_df = crit_df.copy()

rank_df["Score"] = (
    (rank_df["Completion %"] * 0.6) -
    (rank_df["Pending"] * 0.4)
)
...
total_villages = filtered_df["Total No. of Villages under Scheme"].sum()
completed = filtered_df["Villages where ground truthing completed & sent back to SoI"].sum()

progress = (completed / total_villages) * 100 if total_villages > 0 else 0

st.metric("Overall Progress %", f"{progress:.2f}%")
